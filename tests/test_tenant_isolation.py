...
def test_tenant_isolation_save_and_load():
    ...
    assert loaded["node-1"].tenant_id == tid  # type: ignore
...
def test_tenant_isolation_different_tenants():
    ...
    assert "node-A" in nodes_a and "node-B" not in nodes_a
    assert "node-B" in nodes_b and "node-A" not in nodes_b
...
def test_audit_log_immutable():
    ...
    # read history
    hist = get_history("n1", tenant_id=tid)
    assert len(hist) == 1
    assert hist[0]["event_type"] == "node.created"
...
def test_tamper_evidence_integrity():
    init_db()
    tid = "org_tamper"
    ev_id = "ev_orig"
    ...
    # read back
    # (no explicit read function, but we trust store)
    # here we just verify the hash is stored; tamper would change hash
    # This is a placeholder for a more thorough test.
